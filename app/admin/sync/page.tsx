...
import { RESOURCE_FIELDS, RESOURCE_LABELS, autoMapColumns } from "@/app/lib/column-mapper";
  ...
  const parseFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Track which fields Claude mapped (non-null) for badge display
        const aiFields = new Set(
          Object.entries(data.mappings)
            .filter(([, v]) => v !== null)
            .map(([k]) => k)
        );
        setAiMappedFields(aiFields);
        setAiReasoning(data.reasoning ?? null);
      } else {
        // Fall back to rule-based mapping
        const fallbackRes = "lease-comps" as Resource;
        setResource(fallbackRes);
        setMappings(autoMapColumns(hdrs, fallbackRes));
      }
    } catch {
      const fallbackRes = "lease-comps" as Resource;
      setResource(fallbackRes);
        ...
        <div style={{ marginBottom: "3rem" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 16, marginBottom: "1.2rem", flexWrap: "wrap" }}>
            ...
            {!aiMapping && aiMappedFields.size > 0 && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 20, padding: "2px 10px", fontSize: "0.78rem", color: "#15803d", fontWeight: 600 }}>
                ✦ AI-mapped
              </span>
            )}
          ...
          {aiMapping ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "3rem 1rem", gap: "1rem", color: "#475569" }}>
              <div style={{ fontSize: "1.5rem", animation: "spin 1.2s linear infinite" }}>⟳</div>
              <div style={{ fontWeight: 600 }}>Claude is reading your file…</div>
              <div style={{ fontSize: "0.84rem", color: "#94a3b8" }}>Detecting resource type and mapping columns</div>
              <style>{`@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }`}</style>
            </div>
          ) : (
            <>
              {aiReasoning && (
                <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 8, padding: "8px 14px", marginBottom: "1rem", fontSize: "0.82rem", color: "#166534" }}>
                  ...
                  {headers.map((h) => {
                    ...
                    return (
                      <tr key={h} style={{ borderBottom: "1px solid #e2e8f0" }}>
                        <td style={{ padding: "6px 12px", fontFamily: "monospace", color: mapped ? "#15803d" : "#dc2626" }}>
                          {h}
                          {wasAiMapped && mapped && (
                            <span title="Mapped by Claude" style={{ marginLeft: 6, fontSize: "0.7rem", background: "#dcfce7", color: "#15803d", borderRadius: 4, padding: "1px 5px", fontFamily: "sans-serif", fontWeight: 600 }}>AI</span>
                          )}
                        </td>
